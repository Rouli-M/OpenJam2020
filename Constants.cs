public static class Constants
{
    public const float speedMultiplier = 1.5f;
    public const int DESIGN_WIDTH = 1280;
    public const int DESIGN_HEIGHT = 720;

    public const int SPACE_HEIGHT = Constants.DESIGN_HEIGHT * 30;//8
    public const int SKY_HEIGHT = Constants.DESIGN_HEIGHT * 3 + 400;//5
    public const int TREES_HEIGHT = 720 * 2 + 400;//4

    public const int PREHISTORY_LENGHT = DESIGN_WIDTH * 40; //40;
    public const int PREHISTORY_LENGHT_END = PREHISTORY_LENGHT / 2;
    public const int MIDDLEAGE_LENGHT = DESIGN_WIDTH * 30;//30;
    public const int FUTURE_LENGHT = DESIGN_WIDTH * 100;
}